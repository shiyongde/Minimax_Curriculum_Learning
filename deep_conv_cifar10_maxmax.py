            ...
            if len(passed_index) <= n_train:
                ...
                for iters in range(epoch_iters):
                    ...
                    for batch in iterate_minibatches(X_train[center_nn], Y_train[center_nn], 200, shuffle=False, augment=False):
                        inputs, targets = batch
                        loss_vec_center = np.append(loss_vec_center, 1-loss_fn(inputs, targets))
...
if __name__ == '__main__':
    if ('--help' in sys.argv) or ('-h' in sys.argv):
        ...
        numpy.savetxt('cifar10_convnet_cLearn_maxmax_result.txt', output_seq)
        ...
        plt.savefig('cifar10_convnet_cLearn_maxmax.eps', format = 'eps', bbox_inches='tight')
        plt.show()