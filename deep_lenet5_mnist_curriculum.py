...
def optimization_mnist(learning_rate=2e-2, loss_weight = 1.2e+6, curriculum_rate=0.03, 
                        n_curriculum_epochs=40, epoch_iters = 50, stain_factor = 100.0, 
                        minibatch_size = 50, num_cluster = 1000,
                        batch_size=16, k = 4, func = 'concavefeature', func_parameter = 0.5):
#loss_weight = 1.2e+6
    """
    ...
    for curriculum_epoch in range(n_curriculum_epochs):
        ...
        if len(passed_index) <= n_train:
            ...
            stain_weight = numpy.power(center_pass_normalized, -1/stain_factor)
            # Update kmeans result and submodular function
            if curriculum_epoch % 20 == 0 and curriculum_epoch <= 200:
                start_index = 0
                train_fea = ()
                ...
                #print(numpy.histogram(labels_, bins=num_cluster))
                labels_weight = numpy.array([len(numpy.where(labels_==i)[0]) for i in numpy.unique(labels_)])
                labels_weight = numpy.divide(labels_weight,float(numpy.max(labels_weight)))
                minGain, sinGain, optSubmodular = initSubmodularFunc(cluster_centers_, k)
            for iters in range(epoch_iters):

                 # compute loss
                submodular_start_time = timeit.default_timer()
                loss_vec_center = loss_model(center_nn)
                loss_vec_center *= stain_weight * labels_weight * (loss_weight / num_cluster)
                all_loss = sum(loss_vec_center)
                topkLoss = sum(numpy.partition(loss_vec_center, -k)[-k:])
                if curriculum_epoch % 10 == 0 and curriculum_epoch > 200:
                    #topkIndex = numpy.argpartition(loss_vec_center, k)[:k]
                    #topkIndex = numpy.random.choice(num_cluster, k, replace=False, p=labels_weight/sum(labels_weight))
                    train_index = numpy.random.choice(n_train, 72, replace=False)
                    #train_index = numpy.random.choice(numpy.setxor1d(numpy.arange(n_train), passed_index).astype(int), 24, replace=False)
                else:
                ...
                submodular_time += (timeit.default_timer() - submodular_start_time)
...
if __name__ == '__main__':
    ...
    numpy.savetxt('mnist_lenet5_cLearn_k+8_result.txt', output_seq)
    ...
    plt.savefig('mnist_lenet5_cLearn_k+8.eps', format = 'eps', bbox_inches='tight')
    plt.show()